...
use smithay::{
    ...
    desktop::space::{RenderElement, Space, SurfaceTree},
    ...
    T: Texture + 'static,
{
    ...
    fn accumulated_damage(&self, _: Option<(&Space, &Output)>) -> Vec<Rectangle<i32, Logical>> {
        ...
        for digit in value_str.chars().map(|d| d.to_digit(10).unwrap()) {
            let damage = damage
                ...
                .flat_map(|x| {
                    x.intersection(Rectangle::from_loc_and_size(
                        Point::from((offset_x as i32, 0)) + location,
                        (22, 35),
                    ))
                })
                .map(|mut x| {
            ...
            frame.render_texture_from_to(
                ...
                Rectangle::from_loc_and_size(
                    Point::from(((offset_x as f64) * scale, 0.0)) + location.to_f64().to_physical(scale),
